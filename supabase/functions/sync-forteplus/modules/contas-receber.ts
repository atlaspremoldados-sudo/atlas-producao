// supabase/functions/sync-forteplus/modules/contas-receber.ts
//
// Sincroniza /api/v1/financas_receber/ → forteplus.contas_receber
//
// ✅ Endpoint + field names confirmados em 18/05/2026 via fields-discovery.ts.
// Naming do Atlas mantém "contas_receber" (user-friendly); no FP é "financas_receber".
// Campos do FP usam prefixo `fn_*` (financas) compartilhado com financas_pagar.
...
const RECURSO = 'contas_receber';
const WATERMARK_INICIAL = '2025-05-12';
const ENDPOINT = '/financas_receber/';

interface FPFinanca {
  id: number;
  fn_pessoa?: number | { id: number };
  fn_natureza?: number | { id: number; nt_codigo?: string; nt_descricao?: string };
...
export async function syncContasReceber(
  ...
  try {
    ...
    for (const c of items) {
      // fn_doc_origem é numérico mas tipado como string às vezes
      const movId = c.fn_doc_origem != null && c.fn_tipo_doc_origem === 'PED'
        ? Number(c.fn_doc_origem) || null
        : null;
      ...
      if (upErr) {
        console.error(`[contas_receber] upsert falhou id=${c.id}: ${upErr.message}`);
        continue;
      }
      upsertados++;
